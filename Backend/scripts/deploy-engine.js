const { ethers } = require("hardhat");
const fs = require("node:fs");
const path = require("node:path");

async function main() {
  console.log("🚀 Deploying GameEngine...");

  const GameEngine = await ethers.getContractFactory("GameEngine");
  const engine = await GameEngine.deploy();
  await engine.deployed();

  const address = engine.address;
  console.log("✅ GameEngine deployed to:", address);

  const outputPath = path.join(__dirname, "..", "frontend", "src", "constants", "addresses.ts");

  let existing = {};
  if (fs.existsSync(outputPath)) {
    const raw = fs.readFileSync(outputPath, "utf8");

    // 👇 Match TypeScript export line
    const match = raw.match(/export const CONTRACTS = (\{[\s\S]*?\});/);
    if (match) {
      try {
        // Convert TypeScript object to JSON-safe format
        try {
          existing = JSON.parse(match[1]);
        } catch (e) {
          console.warn("⚠️ Couldn't parse existing CONTRACTS as JSON, starting fresh.");
        }
      } catch (e) {
        console.warn("⚠️ Couldn't parse existing CONTRACTS, starting fresh.");
      }
    }
  }

  // ✅ Update only gameEngineAddress
  existing.gameEngineAddress = address;

  // Regenerate TypeScript file
  const newContent = `// Auto-generated by deploy script
export const CONTRACTS = ${JSON.stringify(existing, null, 2)};
`;

  fs.writeFileSync(outputPath, newContent);
  console.log("📁 Updated: addresses.ts ✅");
}

main().catch((error) => {
  console.error("❌ Deployment failed:", error);
  process.exitCode = 1;
});
